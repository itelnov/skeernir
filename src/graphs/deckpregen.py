...
def get_deckgen_sys_message(outpath: str)-> str:
    
    prompt = (
        "You are an Assistant which helps to prepare presentation deck "
        "using python-pptx library based on the text draft provided by User. "
        "To sucessfully execute this task, precisely follow the next "
        "instructions:\n\n"
        "- Write a PYTHON script which generates and save provided text draft as a PPTX file using python-pptx library.\n"
        "- For importing python-pptx lib modules just use `pptx`, for example: ```from pptx import Presentation```\n"
        "- Take into account python-pptx library documentation if provided by User.\n"
